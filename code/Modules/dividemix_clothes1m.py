...
from .funcs import cluster_kmeans, get_average_value
...
class DivideMixTrainCifar:
    ...
    def eval_prob_clean_for_dataset_division(self, net, data_loader):
        ...
        net.eval()
        num_data = 1e6 if args.num_samples == -1 else args.num_samples
        losses = torch.zeros(int(min(args.batch_size * args.num_batches, num_data)))
        with torch.no_grad():
        ...
        for epoch in range(epochs + 1):
            ...
            if epoch < epochs_warmup:
                ...
                warmup_dataset = self.dataset_getter.get_warmup_dataset(
                    num_samples=int(2 * args.batch_size * args.num_batches))
                warmup_loader = torch.utils.data.DataLoader(warmup_dataset, batch_size=2 * args.batch_size,
                                                            shuffle=True,
                                                            num_workers=args.num_workers, drop_last=False)
            ...
            # Train dividemix.
                ...
                eval_dataset_1 = self.dataset_getter.get_eval_dataset(
                    num_samples=int(args.batch_size * args.num_batches))
                eval_loader_1 = torch.utils.data.DataLoader(eval_dataset_1, batch_size=args.batch_size, shuffle=False,
                                                            num_workers=args.num_workers, drop_last=False)
                prob_1 = self.eval_prob_clean_for_dataset_division(net_1, eval_loader_1)
                pred_1 = np.array(prob_1 > args.p_threshold, dtype=bool)

                eval_dataset_2 = self.dataset_getter.get_eval_dataset(
                    num_samples=int(args.batch_size * args.num_batches))
                eval_loader_2 = torch.utils.data.DataLoader(eval_dataset_2, batch_size=args.batch_size, shuffle=False,
                                                            num_workers=args.num_workers, drop_last=False)
                prob_2 = self.eval_prob_clean_for_dataset_division(net_2, eval_loader_2)
                ...
                print('Train Net1')
                labeled_dataset, unlabeled_dataset = self.dataset_getter.get_divide_datasets(data=eval_dataset_2.data,
                                                                                             pred=pred_2, prob=prob_2)
                ...
                print('\nTrain Net2')
                labeled_dataset, unlabeled_dataset = self.dataset_getter.get_divide_datasets(data=eval_dataset_1.data,
                                                                                             pred=pred_1, prob=prob_1)
                labeled_loader = torch.utils.data.DataLoader(labeled_dataset, batch_size=args.batch_size, shuffle=True,
                                                             num_workers=args.num_workers, drop_last=False)
                unlabeled_loader = torch.utils.data.DataLoader(unlabeled_dataset, batch_size=args.batch_size,
                                                               shuffle=True, num_workers=args.num_workers,
    ...
    def train_noisy(self, epochs):
        ...
        for epoch in range(epochs + 1):
            ...
            if epoch == epochs / 2:
                lr /= 10
                self.adjust_learning_rate(optimizer_1, lr)
...
class DivideMixClusterNMCETrainCifar(DivideMixTrainCifar):
    ...
    def cluster_features(self, net, current_cluster, eval_loader, cluster_num, epoch=0, interval=1):

        if epoch % interval == 0 or current_cluster is None:
            features_1, targets_1 = self.feature_extractor(net, eval_loader)
            clusters_1 = cluster_kmeans(features_1, cluster_num=cluster_num)
        else:
            clusters_1 = current_cluster
    ...
    def train(self, epochs, epochs_warmup):
        ...
        for epoch in range(epochs + 1):
            ...
            # Train dividemix.
                ...
                eval_loader_1 = torch.utils.data.DataLoader(eval_dataset_1, batch_size=args.batch_size, shuffle=False,
                                                            num_workers=args.num_workers, drop_last=False)
                print("Calculating features for Net1")
                clusters_1 = self.cluster_features(net_1, self.clusters_1, eval_loader_1,
                                                   cluster_num, epoch=epoch - epochs_warmup,
                                                   interval=args.cluster_interval)
                self.clusters_1 = clusters_1
                prob_1 = self.eval_prob_clean_for_dataset_division_clusters(net_1, eval_loader_1, clusters_1,
                                                                            cluster_num)
                pred_1 = np.array(prob_1 > args.p_threshold, dtype=bool)
                ...
                eval_loader_2 = torch.utils.data.DataLoader(eval_dataset_2, batch_size=args.batch_size, shuffle=False,
                                                            num_workers=args.num_workers, drop_last=False)
                print("Calculating features for Net2")
                clusters_2 = self.cluster_features(net_2, self.clusters_2, eval_loader_2,
                                                   cluster_num, epoch=epoch - epochs_warmup,
                                                   interval=args.cluster_interval)
                self.clusters_2 = clusters_2
                prob_2 = self.eval_prob_clean_for_dataset_division_clusters(net_2, eval_loader_2, clusters_2,
                                                                            cluster_num)
                pred_2 = np.array(prob_2 > args.p_threshold, dtype=bool)
...
def adjust_learning_rate(optimizer, epoch, num_epochs, lr_init, cosine=True, lr_decay_rate=0.1):
    ...
    decay_times = 3
    decay_interval = num_epochs // (decay_times + 1)
    decay_coefficient = lr_decay_rate ** (epoch // decay_interval)
    lr *= decay_coefficient
