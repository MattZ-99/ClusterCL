...
def fmix_loss(pred, y1, index, lam, train=True, reformulate=False):
    r"""Criterion for fmix
    Args:
        pred: If trained, mixed input. If not train, standard input
    ...
    if train and not reformulate:
        ...
        y1 = y1.max(1)[1]
        y2 = y2.max(1)[1]
        return F.cross_entropy(pred, y1) * lam + F.cross_entropy(pred, y2) * (1 - lam)
    else:
        return F.cross_entropy(pred, y1)
...
def make_low_freq_image(decay, shape, ch=1):
    ...
    :param decay: Decay power for frequency decay prop 1/f**d
...
def sample_mask(alpha, decay_power, shape, max_soft=0.0, reformulate=False):
    """ Samples a mean lambda from beta distribution parametrised by alpha, creates a low frequency image and binaries
